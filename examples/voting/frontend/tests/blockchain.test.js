...
const PROOF_URL = '/api/services/voting/v1/wallets/info?pub_key=';
...
mock
  .onGet(
    `${PROOF_URL}ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600`
  )
  .replyOnce(200, walletProof);

// Mock `createWallet` transaction
mock
  .onPost(TRANSACTION_URL, {
    protocol_version: 0,
    service_id: 128,
    message_id: 2,
    signature:
      'c28cc03f7b2bb41cac2c83896be31a293594100e8fdced2d439165f7e9227b271b464408c694df887548971db70b2cf4f287f0781f1a0e9dfe9bfa0f0b80b70a',
    body: {
      pub_key:
        'ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600',
      name: 'John Doe',
    },
  })
  .replyOnce(200);
...
mock
  .onGet(
    `${TRANSACTION_EXPLORER_URL}473fab385340f46b1e89f03124d05d849d7948ef11bc20ae569c96a15052704c`
  )
  .replyOnce(200, {
    type: 'in-pool',
  });
...
mock
  .onPost(TRANSACTION_URL, {
    ...
    message_id: 1,
    signature:
      'a1495b35248f7aefce93d9b7af431e2de6cc1ee523471a929b8a49045b0cf89f9a151627abfdc45671d866ce2a1b0e1282869ba233b1419acc87c5a5b064ef08',
    body: {
      amount: '50',
      pub_key:
        'ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600',
...
mock
  .onGet(
    `${TRANSACTION_EXPLORER_URL}e4e1fa9f9dc0dad5763416d0f048b0d4179775de0249fc0927ceaf90b5beb16a`
  )
  .replyOnce(200, {
    type: 'in-pool',
  });
...
mock
  ...
  .replyOnce(200, addFundsTxNotAccepted);

mock
  .onGet(
    `${PROOF_URL}ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600`
  )
  .replyOnce(200, addFundsTxAccepted);
...
mock
  .onPost(TRANSACTION_URL, {
    ...
    message_id: 0,
    signature:
      '49cab16b383c9d8117fdf0bfed9bd1cc88f0638b5b50067084fbcdad64fcaf0b1240d7ff47c9009a6b3960b10bd882d362cae91a2753c2d68896a136601a8501',
    body: {
      amount: '100',
...
mock
  .onGet(
    `${TRANSACTION_EXPLORER_URL}193d2932818a906b670833e0b0dcd5d16a045b73f7afeb9ea35d91856de204cf`
  )
  .replyOnce(200, {
    type: 'in-pool',
  });
...
mock
  ...
  .replyOnce(200, transferTxNotAccepted);

mock
  .onGet(
    `${PROOF_URL}ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600`
  )
  .replyOnce(200, transferTxAccepted);
...
describe('Interaction with blockchain', () => {
  ...
  it('should get wallet proof and verify it', async () => {
    const keyPair = {
      publicKey:
        'ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600',
      secretKey:
        '925e9a5787d97d720bf16adff5c6d3ebf81cf27b61a474e1cbc97f4f80dce4e0ba78f4566a075958770ffd514cde99ed56bdb349fd95464a0b3ee1fb2459c600',
    };
  ...
  it('should create new wallet', async () => {
    ...
    await expect(Vue.prototype.$blockchain.createWallet(keyPair, name, '0'))
      .resolves;
  });
  ...
  it('should transfer funds', async () => {
    ...
    const receiver =
      'bdf69b79d03f4debdcc0eb1ee36074094930badb17ee22888a7728ab42e5e493';
    const amountToTransfer = '100';
    const seed = '15549015379304915022';

    await expect(
      Vue.prototype.$blockchain.transfer(
        keyPair,
        receiver,
        amountToTransfer,
        seed
